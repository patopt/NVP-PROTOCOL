        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    // NVP OS (beta) — simulated iOS-like desktop inside the app
                    VStack(alignment: .leading, spacing: 8) {
                        HStack(spacing: 8) {
                            Text("NVP OS").font(.headline).foregroundColor(Theme.text)
                        ...
                        Toggle(isOn: Binding(get: { app.nvpOSOn }, set: { app.setNvpOS($0) })) {
                            Text("Activer NVP OS").foregroundColor(Theme.text).font(.callout)
                        }.tint(Theme.green)
                        Text("Un bureau tactile façon iOS, directement dans l'app : tes apps NVP (Worker, Chat, Studio, Wallet…) et des apps web, réunies au même endroit. Rester dans NVP OS garde le worker actif (iOS met les apps en pause en arrière-plan). En bêta : certaines apps natives ne peuvent pas s'ouvrir à cause des restrictions d'Apple.")
                            .font(.caption2).foregroundColor(Theme.muted)
                        if app.nvpOSOn {
                            Button { app.showNVPOS = true } label: {
                                Label("Ouvrir NVP OS", systemImage: "square.grid.2x2.fill")
