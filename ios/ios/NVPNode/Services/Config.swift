...
enum Config {
    ...
    /// NVP OS (beta): a simulated, iOS-like desktop INSIDE the worker app. Keeping
    /// the user inside the app means the worker keeps running (iOS suspends apps in
    /// the background, so "stay in the app" is the on-device way to keep earning).
    static var nvpOSEnabled: Bool {
        get { UserDefaults.standard.bool(forKey: "nvp_os_enabled") }
        set { UserDefaults.standard.set(newValue, forKey: "nvp_os_enabled") }
    }
